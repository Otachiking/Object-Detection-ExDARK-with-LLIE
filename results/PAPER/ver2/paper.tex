...
\begin{abstract}
Penelitian ini mengevaluasi dampak integrasi metode \textit{Low-Light Image Enhancement} (LLIE) sebagai tahap pra-pemrosesan terhadap kinerja detektor objek YOLOv11n pada dataset Exclusively Dark (ExDARK). Tiga metode LLIE mutakhir — HVI-CIDNet, RetinexFormer, dan LYT-Net — dibandingkan secara sistematis terhadap skenario \textit{baseline} (citra mentah) menggunakan metrik kualitas citra tanpa referensi (NIQE, BRISQUE), pelestarian tepi struktural (EPI), akurasi deteksi (mAP@0.5, Precision, Recall), dan biaya komputasi (GFLOPs, latensi). Hasil eksperimen pada 7.363 citra ExDARK mengungkapkan anomali empiris: skenario \textit{baseline} justru mempertahankan keunggulan pada mAP@0.5 (0,5997) dan Recall (0,5569) dibandingkan seluruh skenario LLIE, meskipun metode-metode tersebut berhasil meningkatkan kualitas perseptual secara signifikan. Analisis menggunakan Spatial Mean Activation Map dan Eigen-CAM pada lapisan C2PSA menunjukkan bahwa artefak LLIE mendistorsi distribusi variansi fitur, mengalihkan atensi jaringan ke noise frekuensi tinggi sebagai batas spasial palsu. Temuan ini mengindikasikan bahwa YOLOv11n memiliki kapabilitas ekstraksi fitur yang memadai pada kondisi minim cahaya, sehingga penambahan pra-pemrosesan LLIE cenderung kontraproduktif terhadap akurasi sekaligus menambah beban latensi rata-rata 101–356 ms per citra.
\end{abstract}
...
Untuk mengukur efektivitas metode tersebut, mayoritas studi LLIE hanya berhenti pada evaluasi kualitas visual menggunakan metrik \textit{Full-Reference} seperti PSNR dan SSIM. Evaluasi semacam ini tidak dapat diaplikasikan secara objektif pada dataset dunia nyata seperti ExDARK yang bersifat \textit{unpaired} (tanpa referensi terang) \cite{loh2019exdark,liu2021benchmark}. Oleh karena itu, penelitian ini mengadopsi metrik buta (\textit{No-Reference}) seperti \textit{Natural Image Quality Evaluator} (NIQE) \cite{mittal2013niqe} dan \textit{Blind/Referenceless Image Spatial Quality Evaluator} (BRISQUE) \cite{mittal2012brisque}. Lebih lanjut, pelestarian struktur batas objek dievaluasi menggunakan metrik \textit{Edge Preservation Index} (EPI) guna memastikan bahwa perbaikan pencahayaan tidak merusak fitur spasial yang relevan bagi detektor.
...
Arsitektur detektor yang digunakan difokuskan pada varian YOLOv11n \cite{khanam2024yolov11}. Pemilihan varian dengan kompleksitas terendah ini dirancang untuk mengisolasi variabel eksperimen: model berkapasitas kecil cenderung lebih sensitif terhadap kualitas fitur dan gradien input dibandingkan model berkapasitas besar \cite{dobrzycki2025freezing}, sehingga fluktuasi akurasi lebih mencerminkan kualitas citra yang dipengaruhi oleh LLIE. Selain itu, penggunaan model yang ringan merupakan kompensasi terhadap beban komputasi tambahan yang ditimbulkan oleh tahap pra-pemrosesan citra.

Pelatihan model YOLOv11n dilakukan secara konsisten pada keempat skenario selama 50 epochs dengan ukuran batch 16. Optimasi jaringan menggunakan algoritma AdamW \cite{loshchilov2017sgdr} dengan learning rate awal sebesar 0.005 dan weight decay 0.001. Guna meningkatkan kemampuan generalisasi detektor, strategi augmentasi data diaktifkan secara dinamis dengan probabilitas Mosaic (1.0), Mixup (0.2), serta Erasing (0.2).
...
\begin{figure}[htbp]
    ...
    \caption{Perbandingan visual hasil LLIE pada enam sampel \textit{low-light}: (a) eksterior kendaraan, (b) interior minim cahaya, (c) \textit{mixed-light} dengan lampu meja, (d) ruangan gelap dengan cahaya alami minimal, (e) \textit{outdoor} hitam putih, (f) kelompok orang dengan lilin. Kolom: Raw, HVI-CIDNet, RetinexFormer, LYT-Net.}
...
Hasil akhir pada Table~\ref{tab:detection_metrics} memperlihatkan anomali empiris yang sejalan dengan temuan Wu et al.~\cite{wu2024llieeffect}: skenario \textit{baseline} mempertahankan keunggulan pada mAP@0.5 (0.5997) dan \textit{Recall} (0.5569), meskipun \textit{Precision} LYT-Net (0.6740) sedikit melampaui \textit{baseline} (0.6644). Temuan ini kontras dengan literatur terdahulu~\cite{wang2023yolov5lowlight,shovo2024lowlight} yang menunjukkan bahwa pra-pemrosesan LLIE cenderung mampu meningkatkan akurasi deteksi. Perbedaan ini kemungkinan disebabkan oleh evolusi kapabilitas arsitektur: pada YOLOv3 atau YOLOv5, kapasitas ekstraksi fitur yang terbatas memang memerlukan LLIE untuk memperjelas piksel objek. Sebaliknya, YOLOv11 telah dilengkapi modul C3k2, SPPF, dan atensi spasial C2PSA~\cite{khanam2024yolov11} yang dapat membaca gradien murni tanpa bantuan LLIE, sehingga artefak yang diinjeksikan justru berpotensi mendisorientasi fokus atensi jaringan tersebut.
...
\begin{figure}[htbp]
    ...
    \caption{Komparasi prediksi lokalisasi pada enam sampel yang sama dengan Fig.~\ref{fig:llie_samples}. Kolom: \textit{Ground Truth} (GT), Baseline, HVI-CIDNet, RetinexFormer, LYT-Net.}
...
Penelitian ini mengekstraksi peta aktivasi semantik pada lapisan C2PSA — lapisan atensi spasial terakhir sebelum \textit{Detection Head} pada YOLOv11 — menggunakan dua pendekatan komplementer: \textit{Spatial Mean Activation Map} yang merata-ratakan respons seluruh saluran untuk melihat distribusi intensitas fitur secara global, dan Eigen-CAM yang mengekstraksi arah variansi dominan (PC1) secara \textit{class-agnostic} \cite{muhammad2020eigencam} melalui \textit{Singular Value Decomposition} (SVD) untuk menyoroti region spasial yang paling memicu atensi jaringan.
...
Hal ini dibuktikan secara kualitatif melalui tiga skenario observasi pada Gambar~\ref{fig:eigen_cam}. Pada kasus \textbf{(a)}, scene kucing dalam kegelapan ekstrem: seluruh skenario menunjukkan adanya respons atensi pada area tubuh kucing di C2PSA. Namun pada S2 (HVI-CIDNet), terdapat hotspot saliency yang signifikan mengarah ke objek latar belakang berwarna oranye — yang sesungguhnya hanyalah noise lampu hasil restorasi — sehingga \textit{Detection Head} S2 pun mengklasifikasikannya sebagai \textit{Bottle} (false positive). S3 gagal mendeteksi sama sekali (missed detection), sementara S4 (LYT-Net) berhasil menghasilkan deteksi yang benar dengan peta EigenCAM yang paling terfokus pada area kucing.

Pada kasus \textbf{(b)}, scene dengan lampu meja dan kehadiran orang: seluruh skenario menunjukkan bahwa C2PSA berhasil menangkap fitur orang di scene tersebut, dan \textit{Detection Head} pun menghasilkan prediksi yang cukup baik meskipun tidak seluruh \textit{ground truth} terdeteksi. Lampu meja yang terang menjadi sumber dominasi atensi di Mean Activation, namun EigenCAM tetap mampu mengidentifikasi region orang sebagai arah variansi tersendiri.
...
\caption{Visualisasi peta aktivasi C2PSA pada tiga skenario representatif. Baris atas: Mean Activation Map; Baris bawah: Eigen-CAM (PC1). Kolom: (1) Raw, (2) HVI-CIDNet, (3) RetinexFormer, (4) LYT-Net. (a) Scene kucing: saliency S2 terdistraksi noise oranye \textrightarrow\ false positive Bottle. (b) Scene lampu+orang: fitur orang tertangkap di C2PSA. (c) Scene Menara Eiffel: C2PSA didominasi tower, namun hanya S4 berhasil mendeteksi Boat.}
\label{fig:eigen_cam}
\end{figure*}

Secara keseluruhan, hasil ini mengonfirmasi dua temuan komplementer: (1) artefak LLIE dapat mendistorsi distribusi variansi fitur di C2PSA sehingga komponen PC1 EigenCAM terdistraksi ke noise frekuensi tinggi alih-alih ke fitur objek diskriminatif — yang menjadi salah satu faktor lonjakan false positive pada skenario LLIE; dan (2) keberhasilan deteksi bergantung pada \textit{recognizability} fitur oleh \textit{Detection Head}, bukan semata pada dominasi energi aktivasinya di peta EigenCAM.


\section{Conclusion}

Penelitian ini menunjukkan secara empiris bahwa integrasi algoritma Low-Light Image Enhancement (LLIE) seperti HVI-CIDNet, RetinexFormer, dan LYT-Net sebagai tahap pra-pemrosesan cenderung kontraproduktif terhadap kinerja sistem deteksi objek YOLOv11. Meskipun metode-metode tersebut berhasil menekan area bayangan dan memperbaiki metrik kualitas visual perseptual secara signifikan, proses restorasi ini berdampak negatif pada pelestarian fitur tepi murni, sebagaimana tercermin dari penurunan skor Edge Preservation Index (EPI) dari baseline (1,0000) hingga serendah 0,7503 pada HVI-CIDNet. Analisis kualitatif peta aktivasi C2PSA menggunakan Spatial Mean Activation dan Eigen-CAM mengindikasikan bahwa artefak dan noise frekuensi tinggi dari manipulasi ruang warna mengalihkan fokus atensi jaringan dari target objek ke area latar belakang, sehingga berpotensi memicu peningkatan tingkat False Positive. Selain itu, penambahan modul LLIE juga menambah beban latensi rata-rata 101–356 ms per citra, melampaui ambang \textit{real-time} pada perangkat \textit{edge}. Secara keseluruhan, skenario \textit{baseline} (citra mentah) secara konsisten mengungguli seluruh skenario LLIE pada metrik akurasi utama, mengindikasikan bahwa YOLOv11n memiliki kapabilitas ekstraksi fitur yang cukup memadai untuk menangani citra minim cahaya tanpa bantuan pra-pemrosesan LLIE.
